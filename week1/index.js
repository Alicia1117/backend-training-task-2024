...
// a 是 string
// b 是 number
// c 是 string，第132行為宣告一個c的變數，並賦予字串型別(string)的值為'world'
// d 是 number，第133行宣告一個d的變數，並賦予數字型別(number)的值為456
// e 是 string，第134行宣告一個e的變數，並賦值為變數c和變數d的值相加，而字串型別和數字型別相加時數字型別會被js自動轉型為字串型別，因此最後變數e的值為字串類型，值為'world456'
// f 是 boolean，第135行宣告一個f的變數，並賦予布林型別(boolean)的值為false
// g 是 number，第136行宣告一個g的變數，並賦值為變數d的值自身相加，數字型別的值相加會正常運算，最終的值為數字型別912
// h 是 number，第13行宣告一個h的變數，並賦值為變數f和變數g的值相加，布林型別與數字型別的值相加時，布林值會被轉換為數字(true為1，false為0），因此變數h是數字型別而其值為912(0+912=912)
...
// 第153行為宣告一個numberArr1的變數，並賦予陣列型別的值為[5, 10, 15]
// 第154行為宣告一個numberArr2的變數，並賦予變數numberArr1的值同為陣列型別的值為[5, 10, 15]，此時numberArr2的值為[5, 10, 15]且與numberArr1指向相同的記憶體儲存空間
// 第155行為在numberArr2的陣列最末端插入一個值為20，此時numberArr2的值為[5, 10, 15, 20]，而因為與numberArr1指向相同的記憶體空間，所以numberArr1的值也會變成[5, 10, 15, 20]
// 第156行將numberArr2的值重新賦值，此時numberArr2指向新的記憶體空間而其值為[25, 30, 35]並且不會影響到numberArr1的值
// 第157行為利用console.log查看現在numberArr1的值以及numberArr2的值，numberArr1的值因為在第148行被新增而變成[5, 10, 15, 20],而numberArr2的值在被重新賦值後變為[25, 30, 35]
